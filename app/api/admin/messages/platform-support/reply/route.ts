...
export async function POST(req: Request) {
  try {
    ...
    if (!conversationId || !body) {
      return NextResponse.redirect(new URL("/admin/messages?error=message-required", req.url));
    }
    ...
    if (error || !conversation?.id) {
      return NextResponse.redirect(new URL("/admin/messages?error=thread-not-found", req.url));
    }
    ...
    console.error("[admin/messages/platform-support/reply] failed:", error);
    return NextResponse.redirect(new URL("/admin/messages?error=send-failed", req.url));
  }
}
